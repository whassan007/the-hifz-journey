...
export const TeacherDashboardView = ({
  ...
  return <div className="w-full flex justify-center py-6 px-4">
      <div className="w-full max-w-2xl flex flex-col pt-8">
        ...
        <div className="flex justify-between items-end mb-8">
          ...
          <div className="flex gap-3">
            <button onClick={() => setIsCreatingClass(true)} className="bg-accent hover:bg-amber-600 active:scale-95 transition-all text-white font-bold py-2 px-4 rounded-xl flex items-center gap-2 text-sm shadow-lg">
              <Plus size={18} /> صف جديد
            </button>
            <button onClick={onLogout} className="bg-white/5 border border-white/10 hover:bg-white/10 active:scale-95 transition-all text-white/80 font-bold p-2.5 rounded-xl flex items-center justify-center shadow-lg" title="Logout">
              <LogOut size={18} />
            </button>
          </div>
        </div>
        ...
        <AnimatePresence>
          {isCreatingClass && <motion.div initial={{
          height: 0,
          opacity: 0
        }} animate={{
          height: 'auto',
          ...
          opacity: 0
              ...
              <div className="bg-black/30 border border-white/10 p-6 rounded-3xl backdrop-blur-md">
                ...
                <form onSubmit={handleCreateClass} className="flex flex-col gap-4">
                  <input type="text" placeholder={UI.ui_72} value={newClassName} onChange={e => setNewClassName(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-sm font-medium text-paper outline-none focus:border-accent transition-colors" autoFocus />
                  <input type="text" placeholder={UI.ui_71} value={newClassDesc} onChange={e => setNewClassDesc(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-sm font-medium text-paper outline-none focus:border-accent transition-colors" />
                  <div className="flex gap-3 justify-end mt-2">
                    <button type="button" onClick={() => setIsCreatingClass(false)} className="px-5 py-2.5 rounded-xl font-bold text-sm text-white/60 hover:text-white transition-colors">
                      إلغاء
                    </button>
                    <button type="submit" disabled={!newClassName.trim()} className="bg-accent disabled:opacity-50 disabled:bg-white/10 text-white font-bold py-2.5 px-6 rounded-xl transition-all">
                      إنشاء رمز الانضمام
                    </button>
                  </div>
                </form>
        ...
        <div className="flex flex-col gap-4">
          {classes.length === 0 && !isCreatingClass && <div className="text-center py-16 bg-black/20 rounded-3xl border border-white/5 border-dashed">
          ...
          {classes.map(c => <motion.button key={c.id} onClick={() => setSelectedClass(c)} initial={{
          opacity: 0,
          y: 10
        }} animate={{
          opacity: 1,

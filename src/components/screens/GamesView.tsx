...
import { Gamepad2, Edit3, Puzzle, Link as LinkIcon } from 'lucide-react';
    ...
    <div className="flex flex-col p-6 w-full max-w-4xl mx-auto h-full pb-32">
      ...
      <div className="grid grid-cols-2 gap-4 md:gap-6 mt-4">
        ...
        {[
          ...
          { id: 'mutashabihat', name: 'تحدي المتشابهات', desc: 'التفريق بين الآيات المتشابهة', icon: <Puzzle className="mb-3 text-[#B0A8C9]" size={28} />, color: 'bg-[#B0A8C9]/10 border-[#B0A8C9]/20 hover:bg-[#B0A8C9]/20' },
          { id: 'scramble', name: 'ترتيب الكلمات', desc: 'إعادة بناء الآية بدقة', icon: <LinkIcon className="mb-3 text-[#8DB3C3]" size={28} />, color: 'bg-[#8DB3C3]/10 border-[#8DB3C3]/20 hover:bg-[#8DB3C3]/20' },
        ].map((mode, index) => (
          <motion.button 
            key={mode.id}

...
export const JoinClassModal = ({
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      y: 20
        ...
        <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-white/60 transition-colors">
          <X size={18} />
        </button>

        <AnimatePresence mode="wait">
          {step === 'input' && <motion.div key="input" initial={{
          opacity: 0,
          x: -20
        }} animate={{
          opacity: 1,
              ...
              <form onSubmit={handleSearch} className="flex flex-col gap-4">
                <div className="flex gap-2">
                  <div className="flex-1">
                    <input type="text" placeholder="class-xxx-xxx" value={code} onChange={e => {
                  setCode(e.target.value);
                  setError(null);
                }} className={`w-full bg-black/40 border ${error ? 'border-red-500/50 focus:border-red-500' : 'border-white/10 focus:border-accent'} rounded-xl py-4 px-5 text-center font-mono text-lg text-paper outline-none transition-colors h-[60px]`} autoFocus />
                  </div>
                ...
                <button type="submit" disabled={!code.trim()} className="w-full bg-accent hover:bg-amber-600 disabled:opacity-50 disabled:bg-white/10 text-white font-bold py-4 rounded-xl shadow-lg mt-2 transition-all active:scale-95 text-lg">
                  البحث عن الرمز
                </button>
              </form>
            </motion.div>}

          {step === 'input' && isScanning && <motion.div key="scan" initial={{
          opacity: 0,
          scale: 0.95
        }} animate={{
          opacity: 1,
          ...
          scale: 0.95
              ...
              <button onClick={() => setIsScanning(false)} className="w-full bg-white/10 hover:bg-white/20 text-white font-bold py-4 rounded-xl transition-all">
                إلغاء المسح
              </button>
            </motion.div>}

          {step === 'preview' && foundClass && <motion.div key="preview" initial={{
          opacity: 0,
          x: -20
        }} animate={{
          opacity: 1,
          ...
          x: 20
              ...
              <div className="w-full flex gap-3">
                <button onClick={() => setStep('input')} disabled={isJoining} className="flex-1 bg-white/5 hover:bg-white/10 text-white font-bold py-4 rounded-xl transition-all">
                  إلغاء
                </button>
                <button onClick={confirmJoin} disabled={isJoining} className="flex-[2] bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2">
                  {isJoining ? <span className="animate-pulse">جاري الانضمام...</span> : <>

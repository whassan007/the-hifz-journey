...
const AGE_GROUPS = [
  {
    ...
    name: 'Seedling',
    ageRange: 'Ages 5–9',
    desc: 'Audio-first, playful, forgiving drills.',
    color: 'bg-emerald-500',
    borderColor: 'border-emerald-500',
    lightBg: 'bg-emerald-500/10',
    defaults: { diff: 'Beginner', session: '10–15 min', types: '4 (MCQ, Audio)' }
  },
  {
    id: 'sapling' as AgeGroup,
    name: 'Sapling',
    ageRange: 'Ages 10–15',
    desc: 'Structured practice with gentle competition.',
    color: 'bg-blue-500',
    borderColor: 'border-blue-500',
    lightBg: 'bg-blue-500/10',
    defaults: { diff: 'Intermediate', session: '20–25 min', types: '8 Core types' }
  },
  {
    id: 'rising_tree' as AgeGroup,
    name: 'Rising Tree',
    ageRange: 'Ages 16–25',
    desc: 'Full platform access, intense drills.',
    color: 'bg-orange-500',
    borderColor: 'border-orange-500',
    lightBg: 'bg-orange-500/10',
    defaults: { diff: 'Advanced', session: '30+ min', types: 'All 15 modes' }
  },
  {
    id: 'mighty_oak' as AgeGroup,
    name: 'Mighty Oak',
    ageRange: 'Ages 26+',
    desc: 'Flexible Muraja\'ah for busy lifetstyles.',
    color: 'bg-purple-500',
    borderColor: 'border-purple-500',
    lightBg: 'bg-purple-500/10',
    defaults: { diff: 'Adaptive', session: 'Variable', types: 'All + Teacher Mode' }
  }
];
        ...
        {step === 2 && (
          ...
          >
            ...
            <div className="flex items-center justify-between xl:mb-6 mb-4">
              ...
              <button onClick={() => onComplete(name, 'sapling')} className="text-sm font-medium text-white/50 hover:text-white">Skip</button>
            </div>
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="text-xl font-bold text-paper mb-1">Select your age group</h2>
              <p className="text-[13px] text-paper/60">This helps us personalize your Hifz experience.</p>
            </div>
            ...
            <AnimatePresence>
              {selectedGroup && (
                ...
                >
                  <p className="text-[11px] font-bold uppercase tracking-wider text-teal-400 mb-2">Default Settings</p>
                  <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm mt-2">
                    <span className="text-white/50">Difficulty:</span> <span className="text-right font-medium">{selectedGroup.defaults.diff}</span>
                    <span className="text-white/50">Session:</span> <span className="text-right font-medium">{selectedGroup.defaults.session}</span>
                    <span className="text-white/50">Questions:</span> <span className="text-right font-medium">{selectedGroup.defaults.types}</span>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <p className="text-[11px] text-center text-white/40 mt-4 leading-relaxed px-4">
              You can change this anytime in Settings. Parental verification is required for ages under 16.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
